...
class WorkflowHierarchy(models.Model):
    ...
    # Ajout des nouveaux champs Many2many pour les modèles additionnels
    ...
    def import_hierarchy(self, row):
        ...
        try:
            ...
            # Gestion des Procédures (Many2many)
            procedure_names = row.get('procedure')
            vals['procedure_ids'] = self.with_context(procedure=procedure_names)._find_or_create_m2m_records(
                'workflow.procedure', 'procedure'
            )

            # Gestion des Livrables (Many2many)
            deliverable_names = row.get('deliverable')
            vals['deliverable_ids'] = self.with_context(deliverable=deliverable_names)._find_or_create_m2m_records(
                'workflow.deliverable', 'deliverable'
            )

            # Gestion des Formulations de Tâches (Many2many)
            task_formulation_names = row.get('task_formulation')
            vals['task_formulation_ids'] = self.with_context(
                task_formulation=task_formulation_names)._find_or_create_m2m_records(
...
class WorkflowActivity(models.Model):
    ...
    _description = 'Activités métier (Many2many vers le haut et le bas)'
    ...
    # Inverse relation vers les procédures
    procedure_ids = fields.Many2many('workflow.procedure', 'workflow_activity_procedure_rel', 'activity_id',
                                     'procedure_id', string='Procédures associées')


# --- NOUVEAUX MODÈLES POUR COMPLÉTER LE WORKFLOW ---

class WorkflowProcedure(models.Model):
    _name = 'workflow.procedure'
    _description = 'Procédures de workflow (Many2many vers le haut et le bas)'

    name = fields.Char(string='Nom de la procédure', required=True)
    _sql_constraints = [
        ('name_uniq', 'unique (name)', 'Le nom de la procédure doit être unique !')]

    # Champ Many2many vers les activités
    activity_ids = fields.Many2many('workflow.activity', 'workflow_activity_procedure_rel', 'procedure_id',
                                    'activity_id',
                                    string='Activités associées')

    # Inverse relation vers les livrables
    deliverable_ids = fields.Many2many('workflow.deliverable', 'workflow_procedure_deliverable_rel', 'procedure_id',
                                       'deliverable_id', string='Livrables associés')


class WorkflowDeliverable(models.Model):
    _name = 'workflow.deliverable'
    _description = 'Livrables de workflow (Many2many vers le haut et le bas)'

    name = fields.Char(string='Nom du livrable', required=True)
    _sql_constraints = [
        ('name_uniq', 'unique (name)', 'Le nom du livrable doit être unique !')]

    # Champ Many2many vers les procédures
    procedure_ids = fields.Many2many('workflow.procedure', 'workflow_procedure_deliverable_rel', 'deliverable_id',
                                     'procedure_id',
                                     string='Procédures associées')

    # Inverse relation vers les formulations de tâches
    task_formulation_ids = fields.Many2many('workflow.task.formulation', 'workflow_deliverable_task_formulation_rel',
                                            'deliverable_id', 'task_formulation_id',
                                            string='Formulations de tâches associées')


class WorkflowTaskFormulation(models.Model):
    _name = 'workflow.task.formulation'
    _description = 'Formulation des tâches (Many2many vers le haut)'

    name = fields.Char(string='Description de la tâche', required=True)
    _sql_constraints = [
        ('name_uniq', 'unique (name)', 'La description de la tâche doit être unique !')]

    # Champ Many2many vers les livrables
    deliverable_ids = fields.Many2many('workflow.deliverable', 'workflow_deliverable_task_formulation_rel',
                                       'task_formulation_id', 'deliverable_id', string='Livrables associés')
